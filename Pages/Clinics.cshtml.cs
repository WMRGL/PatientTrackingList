...
namespace PatientTrackingList.Pages
{
    public class ClinicsModel : PageModel
    {
        ...
        private readonly IAppointmentData _appData;
        private readonly IStaffUserData _staffData;
        private readonly IDiseaseData _diseaseData;        
        private readonly ISqlServices _sql;

        public ClinicsModel(ClinicalContext context, IConfiguration config)
        {
            _config = config;
            _context = context;
            _appData = new AppointmentData(_context);
        ...
        public void OnGet(string? clinicianCode, string? diseaseCode, DateTime? startDate, DateTime? endDate)
        {
            try
            {
                ...
                else
                {
                    ...
                    appointments = _appData.GetAppointments(startDate.GetValueOrDefault(), endDate.GetValueOrDefault(), clinicianCode, null);

                    this.startDate = startDate.GetValueOrDefault();
                    this.endDate = endDate.GetValueOrDefault();
                    seenBy = clinicianCode;
                }
            }
            catch (Exception ex)
            {
